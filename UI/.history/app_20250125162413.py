...
# Hardcoded keys
TWELVE_LABS_API_KEY = "tlk_0E7AESG219CKAS20M1B150Z9W7V8"
ZILLIZ_CLOUD_URI = "https://in03-f978dda3f6fa386.serverless.gcp-us-west1.cloud.zilliz.com"
ZILLIZ_CLOUD_API_KEY = "6ef17dec59b05119f0264ee1a8f17e8c999066fbe4715ccd478ee50a56d99eeb60d44eeebf7371b17068b9dcd5e56249ff4d33b9"

# Function to initialize Milvus connection
def initialize_milvus(collection_name, query_embedding_dim):
    connections.connect(
        alias="default",
        uri=ZILLIZ_CLOUD_URI,  # Hardcoded URI
        token=ZILLIZ_CLOUD_API_KEY  # Hardcoded API Key
    )
...
if search_button and query_text:
    ...
    with st.spinner("Generating embeddings from Twelve Labs..."):
        ...
        dummy_ids = list(range(1, 11))  # Simulated IDs for 10 entries
        try:
            collection.insert([dummy_ids, dummy_embeddings])
    ...
    with st.spinner("Performing similarity search..."):
        try:
            ...
            video_urls = {
                1: "https://drive.google.com/file/d/1Sa2Ctk8kQSXYywcMqCar3Fm79U5c4U4i/view?usp=sharing",
                2: "https://drive.google.com/file/d/1Sa2Ctk8kQSXYywcMqCar3Fm79U5c4U4i/view?usp=sharing",
                3: "https://drive.google.com/file/d/1Sa2Ctk8kQSXYywcMqCar3Fm79U5c4U4i/view?usp=sharing"
            }
            st.subheader("Video URL")
            for i, result in enumerate(search_results[0]):
                video_url = video_urls.get(result.id, "Unknown URL")
                st.write(f"[Result {i+1} - Watch Video]({video_url})")
